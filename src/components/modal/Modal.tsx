...
const Modal: React.FC<ModalProps> = ({ show, onClose, data }) => {
  const { pokemonDetails, imageVariant, setImageVariant, section, setSection } =
    usePokemonDetails(data?.url);
  ...
  const renderImagen = useCallback((): JSX.Element => {
    if (pokemonDetails) {
      return (
        <>
          ...
          />
          <div
            onClick={toggleImageVariant}
            className="big-circle-transparent"
          />
        </>
      );
    }
    return (
      <>
        ...
        <div className="mid-circle first" />
        <div className="mid-circle second" />
        <div className="mid-circle third" />
        <div className="small-circle first" />
        <div className="small-circle second" />
      </>
    );
        ...
        return (
          <div className="stats-list">
            {pokemonDetails?.abilities.map(
              (ability: PokemonAbilities, index: number) => (
        ...
        return (
          <div className="stats-list">
            {pokemonDetails?.stats.map((stat: PokemonStat, index: number) => (
              <p key={index}>
                {`${stat.stat.name}: ${stat.base_stat}` || "Not found"}
              </p>
            ))}
          </div>
        );
  ...
  const renderNumber = useCallback((): JSX.Element => {
    return (
      <span className="pokemon-number">N.º{pokemonDetails?.id || "?"}</span>
    );
  }, [pokemonDetails?.id]);
      ...
      <div className="button-container">
        {OPTIONS.map((option: string) => (
          <Button
            ...
            className={`custom-button__modal ${
